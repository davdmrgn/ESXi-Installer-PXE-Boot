...
MAC=$(esxcli network nic get -n vmnic0 | grep 'Virtual Address:' | awk {'print $3'} | sed -r 's/://g' | cut -c 9-12)  # Capture end of MAC address of vmnic0 without colons
esxcli system settings advanced set -o /Misc/PreferredHostName -s esx-$MAC  # Set Hostname based on vmnic0 MAC address
...
vim-cmd hostsvc/datastore/rename datastore1 datastore-$MAC
